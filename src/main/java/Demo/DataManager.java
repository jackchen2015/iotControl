...
public class DataManager extends ManagerModule {
	...
	void addGetServiceListItem(JPanel pCurPanel) {
		// Query Data
		final JPanel pQueryData = new JPanel();
		pQueryData.setLayout(new FlowLayout());
	...
	void updateQueryDataItemWithRawData(final JPanel pQueryData, String strDeviceId) {
		// Update Main Frame
	...
	void updateQueryDataItemNormal(final JPanel pQueryData, String strDeviceId, final Vector<ServiceInfo> vServiceList) {
		// Update Main Frame
		mManager.setVisible(false);
		pQueryData.removeAll();
		...
		pQueryData.add(new JLabel("Service:"));
		// Service List Box
		final JComboBox<String> boxService = new JComboBox<String>();
		pQueryData.add(boxService);
